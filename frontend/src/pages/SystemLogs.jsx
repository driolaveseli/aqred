...
const LEVEL_ROW = {
  ...
  SECURITY: "bg-purple-50/20 dark:bg-purple-900/10",
};
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 md:gap-4 mb-6">
        {[
          { label: "Total",    value: counts.total,    cls: "text-gray-900 dark:text-white" },
          { label: "Info",     value: counts.INFO,     cls: "text-blue-600 dark:text-blue-400" },
          { label: "Warning",  value: counts.WARNING,  cls: "text-amber-600 dark:text-amber-400" },
          { label: "Error",    value: counts.ERROR,    cls: "text-red-600 dark:text-red-400" },
          { label: "Security", value: counts.SECURITY, cls: "text-purple-600 dark:text-purple-400" },
        ].map(s => (
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-3 mb-4">
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className="w-full sm:w-64 pl-9 pr-4 py-2.5 bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-1">
          <Filter size={13} className="text-gray-400 dark:text-gray-500 ml-1" />
          {["", "INFO", "WARNING", "ERROR", "SECURITY"].map(lvl => (
            <button key={lvl} onClick={() => setFilter("level", lvl)}
              className={`px-2.5 py-1.5 text-xs font-medium rounded-lg transition-all ${
                filters.level === lvl ? "bg-violet-600 text-white" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"}`}>
              {lvl || "All"}
            </button>
          ))}
        </div>
        ...
        <select value={filters.module} onChange={e => setFilter("module", e.target.value)}
          className="px-3 py-2.5 bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="font-mono">
            {!loading && logs.map((log, idx) => (
              <tr key={log.id}
                className={`${LEVEL_ROW[log.level] || ""} ${idx !== logs.length - 1 ? "border-b border-gray-50 dark:border-gray-800/60" : ""}`}>
                <td className="px-5 py-3.5">
                  <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-semibold rounded-md ${LEVEL_STYLES[log.level] || "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"}`}>
                    {LEVEL_ICON[log.level]} {log.level}
                  </span>
                </td>
                <td className="px-5 py-3.5 text-xs font-medium text-gray-600 dark:text-gray-400 capitalize">{log.module || "-"}</td>
                <td className="px-5 py-3.5 text-xs text-gray-500 dark:text-gray-400">{log.action || "-"}</td>
                <td className="px-5 py-3.5 text-xs text-gray-600 dark:text-gray-400">
                  {log.user_name ? (
                    <div>
                      <div className="font-medium">{log.user_name}</div>
                      <div className="text-gray-400 dark:text-gray-500">{log.user_role}</div>
                    </div>
                  ) : <span className="text-gray-400 dark:text-gray-500">—</span>}
                </td>
                {isSuperAdmin && (
                  <td className="px-5 py-3.5 text-xs text-gray-500 dark:text-gray-400">
                    {log.company_name || <span className="text-gray-300 dark:text-gray-600">—</span>}
                  </td>
                )}
                <td className="px-5 py-3.5 text-xs text-gray-700 dark:text-gray-300 max-w-xs truncate" title={log.description}>
                  {log.description || "-"}
                </td>
        ...
        {loading && (
          <div className="py-12 text-center text-gray-400 dark:text-gray-500 text-sm">Loading logs...</div>
        )}
        {!loading && logs.length === 0 && (
          <div className="py-12 text-center text-gray-400 dark:text-gray-500">
            <ScrollText size={28} className="mx-auto mb-2 text-gray-300 dark:text-gray-700" />
      ...
      {!loading && (
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 text-center">Showing {logs.length} log entries</p>
      )}
