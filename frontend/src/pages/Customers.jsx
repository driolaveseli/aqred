...
const Customers = () => {
        ...
        <div className="absolute left-0 top-full mt-1.5 bg-white border border-gray-100 rounded-xl shadow-lg z-30 overflow-hidden min-w-[130px]" data-status-popover>
          {STATUSES.map((s) => (
            ...
            >
              ...
              {t(s)}
              {(customer.status || "Active") === s && <CheckCircle size={11} className="ml-auto text-violet-500" />}
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      {/* ── Header ── */}
      <div className="relative overflow-hidden rounded-2xl bg-white border border-gray-100 shadow-sm mb-6">
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-50/60 via-white to-violet-50/30 pointer-events-none" />
        <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-emerald-500 via-teal-400 to-violet-500" />
        <div className="absolute -right-8 -top-8 w-52 h-52 rounded-full bg-gradient-to-br from-emerald-100/40 to-teal-100/20 pointer-events-none" />
        <div className="absolute right-16 -bottom-10 w-36 h-36 rounded-full bg-gradient-to-br from-violet-100/30 to-teal-100/20 pointer-events-none" />
        <div className="absolute left-1/3 -top-6 w-40 h-40 rounded-full bg-emerald-50/30 pointer-events-none" />
        <div className="relative px-6 py-5 flex flex-wrap items-center justify-between gap-y-3">
          <div className="flex items-center gap-4">
            <div className="relative flex-shrink-0">
              <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-300/40">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{t("Manage your customer relationships and contact information")}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={handleExport} className="flex items-center gap-2 px-3.5 py-2 border border-gray-200 bg-white/80 text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-50 active:scale-95 transition-all shadow-sm">
              <Download size={14} /> {t("Export CSV")}
            </button>
            <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-violet-700 text-white text-sm font-semibold rounded-xl hover:from-violet-700 hover:to-violet-800 active:scale-95 transition-all shadow-md shadow-violet-300/40">
          ...
          {[
            {
              ...
              baseBg: "bg-violet-50/40", activeBg: "bg-violet-50/80",
              pct: 100,
            },
            {
            ...
            <button
              ...
              className={`relative text-left px-5 py-4 transition-all duration-150 overflow-hidden group
                ${statusTab === s.tab ? s.activeBg : `${s.baseBg} hover:brightness-[0.97]`}`}
            >
              {statusTab === s.tab && (
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
        <div className="relative w-full sm:flex-1 sm:max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2.5 bg-white border border-gray-200 rounded-xl text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-400/30 focus:border-violet-300 transition-all"
            value={search}
        ...
        <div className="flex items-center gap-0.5 bg-gray-100/90 border border-gray-200/60 rounded-xl p-1 flex-shrink-0">
          {STATUS_TABS.map((tab) => (
            <button
              key={tab}
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                statusTab === tab
                  ? "bg-white text-violet-700 shadow-sm border border-gray-200/80"
        ...
        <div className="flex items-center gap-0.5 bg-gray-100/90 border border-gray-200/60 rounded-xl p-1 flex-shrink-0">
          <button onClick={() => setView("table")} className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${view === "table" ? "bg-white text-violet-700 shadow-sm border border-gray-200/80" : "text-gray-500 hover:text-gray-700"}`}>{t("Table")}</button>
          <button onClick={() => setView("grid")}  className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${view === "grid"  ? "bg-white text-violet-700 shadow-sm border border-gray-200/80" : "text-gray-500 hover:text-gray-700"}`}>{t("Grid")}</button>
        </div>
      </div>
      ...
      {view === "table" ? (
            ...
            <table className="w-full min-w-[760px]">
              ...
              <tbody>
                {filtered.map((c, idx) => (
                  <tr
                    key={c.id}
                    className={`group ${idx !== filtered.length - 1 ? "border-b border-gray-50" : ""}
                      transition-all duration-150 ${selected.has(c.id) ? "bg-violet-50/70" : "hover:bg-violet-50/40"}`}
                  >
                    {/* Row checkbox */}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-sm font-bold shadow-md ring-2
                          ${(c.status || "Active") === "Active" ? "ring-emerald-200" : c.status === "Pending" ? "ring-amber-200" : "ring-gray-200"}
                          ${avatarColor(c.name)}`}>
                    ...
                    <td className="px-5 py-4">
                      <p className="text-xs text-gray-600 flex items-center gap-1"><Mail size={10} /> {c.email}</p>
                      {c.phone && <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><Phone size={10} /> {c.phone}</p>}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity duration-150">
                        <button onClick={() => openEdit(c)} className="p-1.5 text-gray-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-all hover:scale-110"><Edit2 size={14} /></button>
                        <button onClick={() => setDeleteId(c.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all hover:scale-110"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => {
            ...
            return (
              <div key={c.id} className={`relative group bg-white border rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-200 ${selected.has(c.id) ? "border-violet-300 ring-2 ring-violet-100" : "border-gray-100"}`}>
                ...
                <div className="relative px-4 pb-4">
                  <div className="flex items-end justify-between -mt-5 mb-3">
                    <div className={`w-12 h-12 rounded-xl border-2 border-white shadow-lg flex items-center justify-center text-base font-bold flex-shrink-0 ${avatarColor(c.name)}`}>
                      {c.name.charAt(0).toUpperCase()}
                    </div>
                  ...
                  {/* ── Contact details ── */}
                  <div className="space-y-1.5 mb-3 bg-gray-50/80 rounded-xl px-3 py-2.5">
                    ...
                    {c.phone && (
                      <p className="text-xs text-gray-500 flex items-center gap-2">
                        <Phone size={11} className="text-blue-400 flex-shrink-0" />{c.phone}
                      </p>
                    )}
                    {c.address && (
                  ...
                  {/* ── Member since ── */}
                  {c.created_at && (
                    <p className="text-[11px] text-gray-400 mb-3 px-0.5">
                      {t("Member since")} {formatDate(c.created_at)}
                    </p>
                  )}

                  {/* ── Actions ── */}
                  <div className="flex items-center border-t border-gray-100 pt-3 -mx-1">
                    ...
                    <button onClick={() => setDeleteId(c.id)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium text-red-400 hover:bg-red-50 hover:text-red-500 rounded-lg transition-colors">
      ...
      <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-40 transition-all duration-300
        ...
        <div className="flex items-center gap-1 bg-white/95 backdrop-blur-xl
          p-2 rounded-2xl shadow-xl shadow-gray-300/60 border border-gray-200/80 whitespace-nowrap">

          {/* Selection count badge */}
          <div className="flex items-center gap-2 pl-3 pr-3.5 py-1.5 bg-violet-50 border border-violet-200 rounded-xl">
            <span className="text-sm font-black text-violet-700 tabular-nums leading-none">{selected.size}</span>
            <span className="text-xs font-medium text-violet-500 leading-none">{t("selected")}</span>
          </div>

          <div className="w-px h-5 bg-gray-200 mx-1" />

          <button onClick={handleBulkExport}
            className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-gray-500
              hover:text-gray-800 hover:bg-gray-100 rounded-xl transition-all active:scale-95">
            <Download size={14} /> {t("Export")}
          </button>

          <button onClick={() => setBulkDeleteConfirm(true)}
            className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-red-500
              hover:text-red-600 hover:bg-red-50 rounded-xl transition-all active:scale-95">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 overflow-y-auto py-4 sm:py-8 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-auto my-auto">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h2 className="font-bold text-gray-900">{editing ? t("Edit Customer") : t("Add Customer")}</h2>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-1">{t("Delete Customer?")}</h3>
      ...
      {bulkDeleteConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4"><Trash2 size={22} className="text-red-500" /></div>
            <h3 className="font-bold text-gray-900 mb-1">{t("Delete")} {selected.size} {t("customers?")}</h3>
            <p className="text-sm text-gray-500 mb-5">{t("This will permanently remove all selected customers. This action cannot be undone.")}</p>
            <div className="flex gap-3">
