...
/* ─── Status helpers ─────────────────────────────────────────────────────── */
const statusBadgeCls = (s) => s === "Inactive" ? "bg-gray-100 text-gray-500" : "bg-green-50 text-green-600";
...
const Suppliers = () => {
  ...
  const StatusBadge = ({ supplier }) => (
    <div className="relative inline-block" data-status-popover>
      ...
      {statusPopover === supplier.id && (
        <div className="absolute left-0 top-full mt-1.5 bg-white border border-gray-100 rounded-xl shadow-lg z-30 overflow-hidden min-w-[130px]" data-status-popover>
          {["Active", "Inactive"].map((s) => (
            <button
              key={s}
              onClick={() => handleStatusChange(supplier.id, s)}
              className={`w-full text-left px-3 py-2 text-xs flex items-center gap-2 transition-colors
                ${(supplier.status || "Active") === s ? "bg-violet-50 text-violet-700 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${statusDotCls(s)}`} />
              {t(s)}
              {(supplier.status || "Active") === s && <CheckCircle size={11} className="ml-auto text-violet-500" />}
            </button>
          ))}
        </div>
      )}
  ...
  const ProductPills = ({ supplier, max = 2 }) => {
    ...
    if (!all.length) return <span className="text-sm text-gray-400">—</span>;
    return (
      <button
      ...
      >
        {visible.map((p) => (
          <span key={p} className="inline-block px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-600 rounded-full whitespace-nowrap group-hover:bg-gray-200 transition-colors">
            {p}
          </span>
        ))}
        {remaining > 0 && (
          <span className="inline-block px-2 py-0.5 text-xs font-semibold bg-violet-50 text-violet-600 rounded-full whitespace-nowrap group-hover:bg-violet-100 transition-colors">
            +{remaining} more
          </span>
        )}
      </button>
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t("Suppliers")}</h1>
          <p className="text-sm text-gray-500 mt-1">{t("Manage your supplier relationships, contacts, and product sources")}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: t("Total"),      value: suppliers.length, color: "text-violet-600",  tab: "All"      },
          { label: t("Active"),     value: activeCount,      color: "text-emerald-600", tab: "Active"   },
          { label: t("Inactive"),   value: inactiveCount,    color: "text-gray-500",    tab: "Inactive" },
          { label: t("Categories"), value: catCount,         color: "text-blue-600",    tab: null       },
        ].map((s) => (
          <button
            key={s.label}
            ...
            className={`text-left bg-white border rounded-2xl p-5 shadow-sm transition-all duration-200
              ...
              ${s.tab !== null && statusTab === s.tab ? "border-violet-300 ring-2 ring-violet-100" : "border-gray-100"}`}
          >
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-3 mb-3">
        ...
        <div className="flex items-center gap-1 bg-white border border-gray-100 rounded-xl p-1 flex-shrink-0">
          {STATUS_TABS.map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${statusTab === tab ? "bg-violet-600 text-white" : "text-gray-500 hover:text-gray-700"}`}
            >
              {t(tab)}
            </button>
          ))}
        ...
        {/* View toggle */}
        <div className="flex items-center gap-1 bg-white border border-gray-100 rounded-xl p-1 flex-shrink-0">
          <button onClick={() => setView("table")} className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${view === "table" ? "bg-violet-600 text-white" : "text-gray-500 hover:text-gray-700"}`}>{t("Table")}</button>
          <button onClick={() => setView("grid")}  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${view === "grid"  ? "bg-violet-600 text-white" : "text-gray-500 hover:text-gray-700"}`}>{t("Grid")}</button>
        </div>
      </div>
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("Contact")}</th>
                  <SortTh field="category">{t("Category")}</SortTh>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("Products Supplied")}</th>
                ...
                {filtered.map((s, idx) => (
                  <tr
                    ...
                    className={`${idx !== filtered.length - 1 ? "border-b border-gray-50" : ""}
                      transition-colors ${selected.has(s.id) ? "bg-violet-50/60" : "hover:bg-violet-50/30"}`}
                  >
                    ...
                    <td className="px-5 py-4">
                      {s.contact_person
                        ? <p className="text-sm text-gray-700 font-medium">{s.contact_person}</p>
                        : <span className="text-sm text-gray-400">—</span>}
                      {s.email && <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><Mail size={10} />{s.email}</p>}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(s)} className="p-1.5 text-gray-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-colors"><Edit2 size={14} /></button>
                        <button onClick={() => setDeleteId(s.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className={`bg-white border rounded-2xl p-5 hover:shadow-md transition-all ${selected.has(s.id) ? "border-violet-300 ring-2 ring-violet-100" : "border-gray-100"}`}
              >
                {/* Card header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      checked={selected.has(s.id)}
                      onChange={() => toggleSelect(s.id)}
                      className="w-4 h-4 rounded border-gray-300 text-violet-600 focus:ring-violet-500 cursor-pointer flex-shrink-0 mt-0.5"
                    />
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900 text-sm truncate">{s.company_name}</p>
                ...
                <div className="space-y-1.5 mb-4">
                  {s.contact_person && (
                    <p className="text-xs text-gray-600 flex items-center gap-2 font-medium">
                      <Building2 size={12} className="text-gray-400 flex-shrink-0" />{s.contact_person}
                    </p>
                  )}
                  {s.email && (
                    <a href={`mailto:${s.email}`} className="text-xs text-gray-500 flex items-center gap-2 hover:text-violet-600 transition-colors">
                      <Mail size={12} className="text-gray-400 flex-shrink-0" />{s.email}
                    </a>
                  )}
                  {s.phone && (
                    <p className="text-xs text-gray-500 flex items-center gap-2">
                      <Phone size={12} className="text-gray-400 flex-shrink-0" />{s.phone}
                    </p>
                  )}
                  {s.location && (
                    <p className="text-xs text-gray-500 flex items-center gap-2">
                      <MapPin size={12} className="text-gray-400 flex-shrink-0" />{s.location}
                    </p>
                  )}
                </div>
                ...
                {products.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{t("Products Supplied")}</p>
                    <div className="flex flex-wrap gap-1">
                      {products.slice(0, 3).map((p) => (
                        <span key={p} className="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded-full">{p}</span>
                      ))}
                      {products.length > 3 && (
                        <button
                          onClick={() => { setProductsPanel(s); setProductSearch(""); }}
                          className="px-2 py-0.5 text-xs bg-violet-50 text-violet-600 rounded-full font-semibold hover:bg-violet-100 transition-colors"
                        >
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                  <button onClick={() => openEdit(s)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium text-violet-600 hover:bg-violet-50 rounded-lg transition-colors">
                  ...
                  <button onClick={() => setDeleteId(s.id)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium text-red-500 hover:bg-red-50 rounded-lg transition-colors">
      ...
      <p className="text-xs text-gray-400 mt-3">
        ...
        {(statusTab !== "All" || catFilter !== "All") && (
          <span className="ml-1">
            · {t("filtered by")}
            {statusTab !== "All" && <span className="font-medium text-gray-500 ml-1">{t(statusTab)}</span>}
            {statusTab !== "All" && catFilter !== "All" && " &"}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 overflow-y-auto py-4 sm:py-8 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-auto my-auto">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
              <div>
                <h2 className="font-bold text-gray-900">{editing ? t("Edit Supplier") : t("Add Supplier")}</h2>
                <p className="text-xs text-gray-400 mt-0.5">{editing ? t("Update supplier information") : t("Fill in the details to add a new supplier")}</p>
              </div>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t("Company Name")} <span className="text-red-500">*</span></label>
                <input type="text" required className={inputCls} placeholder="e.g. Acme Corp" value={form.company_name} onChange={set("company_name")} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t("Contact Person")}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t("Products Supplied")}</label>
                <input
                  type="text"
                  className={inputCls}
                ...
                />
                <p className="text-xs text-gray-400 mt-1">{t("Comma-separated list of products this supplier provides")}</p>
              </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-1">{t("Delete Supplier?")}</h3>
      ...
      {bulkDeleteConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4"><Trash2 size={22} className="text-red-500" /></div>
            <h3 className="font-bold text-gray-900 mb-1">{t("Delete")} {selected.size} {t("suppliers?")}</h3>
            <p className="text-sm text-gray-500 mb-5">{t("This will permanently remove all selected suppliers. This action cannot be undone.")}</p>
            <div className="flex gap-3">
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900">{productsPanel.company_name}</h3>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {all.length} {all.length === 1 ? t("product") : t("products")} {t("supplied")}
                    </p>
                  </div>
                </div>
                <button onClick={() => setProductsPanel(null)} className="text-gray-400 hover:text-gray-600 mt-0.5 p-1"><X size={18} /></button>
              </div>
                ...
                <div className="px-6 pt-4 pb-2 flex-shrink-0">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-9 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20"
                      value={productSearch}
              ...
              <div className="overflow-y-auto flex-1 px-6 py-4">
                {filtered.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-6">{t("No products match your search.")}</p>
                ) : (
                  <ul className="space-y-1">
                    {filtered.map((p, i) => (
                      <li key={p} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                        <span className="w-6 h-6 bg-violet-50 rounded-lg flex items-center justify-center text-xs font-bold text-violet-500 flex-shrink-0">
                          {productSearch ? <Package size={11} className="text-violet-400" /> : i + 1}
                        </span>
                        <span className="text-sm text-gray-700 font-medium flex-1">{p}</span>
                      </li>
                    ))}
                  </ul>
                )}
