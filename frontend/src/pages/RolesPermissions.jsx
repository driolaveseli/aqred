...
const PERM_CATEGORIES = [
  { label: "Core",           icon: LayoutDashboard, color: "text-violet-600", iconBg: "bg-violet-100", perms: ["View Dashboard"] },
  { label: "People",         icon: Users,           color: "text-blue-600",   iconBg: "bg-blue-100",   perms: ["Manage Employees", "User Management"] },
  { label: "Operations",     icon: ShoppingCart,    color: "text-indigo-600", iconBg: "bg-indigo-100", perms: ["Manage Customers", "Manage Products", "Manage Orders", "Manage Suppliers"] },
  { label: "Finance",        icon: DollarSign,      color: "text-emerald-600",iconBg: "bg-emerald-100",perms: ["View Sales", "Manage Payments", "Manage Invoices", "Export Reports"] },
  { label: "Inventory",      icon: Package,         color: "text-amber-600",  iconBg: "bg-amber-100",  perms: ["View Inventory", "Manage Inventory"] },
  { label: "Analytics",      icon: BarChart2,       color: "text-teal-600",   iconBg: "bg-teal-100",   perms: ["View Reports", "View System Logs"] },
  { label: "Administration", icon: Settings,        color: "text-red-600",    iconBg: "bg-red-100",    perms: ["Manage Roles", "System Settings"] },
];
...
const BUILT_IN_META = {
  admin: {
    ...
    selectedCls: "border-2 border-red-200 ring-2 ring-red-100/60 shadow-lg shadow-red-100/40",
    icon: Shield,
  },
  manager: {
    name: "Manager",
    ...
    selectedCls: "border-2 border-violet-200 ring-2 ring-violet-100/60 shadow-lg shadow-violet-100/40",
    icon: UserCheck,
  },
  employee: {
    name: "Employee",
    ...
    selectedCls: "border-2 border-blue-200 ring-2 ring-blue-100/60 shadow-lg shadow-blue-100/40",
    icon: User,
  },
};
...
const CUSTOM_META = {
  ...
  selectedCls: "border-2 border-teal-200 ring-2 ring-teal-100/60 shadow-lg shadow-teal-100/40",
  icon: Shield,
};
  ...
  return (
    <div>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white border border-gray-100 shadow-sm mb-6">
        <div className="absolute inset-0 bg-gradient-to-br from-violet-50/60 via-white to-red-50/20 pointer-events-none" />
        <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-red-400 via-violet-500 to-blue-500" />
        <div className="absolute -right-8 -top-8 w-52 h-52 rounded-full bg-gradient-to-br from-violet-100/40 to-red-100/20 pointer-events-none" />
        <div className="absolute right-20 -bottom-10 w-32 h-32 rounded-full bg-gradient-to-br from-blue-100/30 to-violet-100/20 pointer-events-none" />
        <div className="relative px-6 py-5 flex flex-wrap items-center justify-between gap-y-3">
          <div className="flex items-center gap-4">
            <div className="relative flex-shrink-0">
              <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-red-500 rounded-2xl flex items-center justify-center shadow-lg shadow-violet-300/40">
                <Shield size={22} className="text-white" />
              </div>
              <span className="absolute -bottom-1 -right-1 w-4 h-4 bg-violet-400 border-2 border-white rounded-full" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-gray-900">{t("Roles & Permissions")}</h1>
              <p className="text-xs text-gray-400 mt-0.5">{t("Configure role-based access control for your team")}</p>
            </div>
          </div>
          <button
            onClick={() => { setNewRoleName(""); setNewRolePerms([]); setShowModal(true); }}
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-violet-700 text-white text-sm font-semibold rounded-xl hover:from-violet-700 hover:to-violet-800 active:scale-95 transition-all shadow-md shadow-violet-300/40 flex-shrink-0"
          >
            ...
            return (
              <button
                ...
                className={`group w-full text-left rounded-2xl transition-all duration-150 overflow-hidden
                  ${active ? m.selectedCls : "bg-white border border-gray-100 hover:border-gray-200 hover:shadow-md shadow-sm"}`}
              >
                {/* Gradient accent bar */}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-0.5">
                        <span className="text-sm font-bold text-gray-900 capitalize">{m.name}</span>
                        {m.locked && <Lock size={10} className="text-gray-400" />}
                        {!BUILT_IN_META[roleKey] && (
                          <span className="text-[10px] font-semibold text-teal-600 bg-teal-50 px-1.5 py-0.5 rounded-full border border-teal-100">Custom</span>
                        )}
                      </div>
                      <p className="text-[11px] text-gray-400 leading-relaxed line-clamp-2">{m.desc || "Custom role"}</p>
                    </div>
                  </div>
                  {/* Permission progress */}
                  <div className="mt-3">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Access</span>
                      <span className={`text-[11px] font-bold ${active ? "text-gray-700" : "text-gray-400"}`}>
            ...
            {PERM_CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat.label} className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-gray-300 ml-auto">{cat.perms.length} perms</span>
                </div>
              );
            })}
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-gray-900 capitalize">{meta.name} {t("Permissions")}</h3>
                    {locked && (
                  ...
                  <div className="flex items-center gap-2.5 mt-0.5">
                    <span className="text-xs text-gray-400">{currentPerms.length}/{PERMISSIONS.length} {t("enabled")}</span>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {!isBuiltIn && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 text-sm text-red-500 border border-red-200 bg-red-50 rounded-xl hover:bg-red-100 transition-colors"
                  >
                    <Trash2 size={13} /> {t("Delete")}
                  </button>
                )}
                {locked ? (
                  <div className="flex items-center gap-1.5 px-3 py-2 text-sm text-gray-400 bg-gray-50 border border-gray-200 rounded-xl">
                  ...
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-violet-700 text-white text-sm font-semibold rounded-xl hover:from-violet-700 hover:to-violet-800 disabled:opacity-60 active:scale-95 transition-all shadow-md shadow-violet-300/40"
                  >
              ...
              return (
                <div key={cat.label}>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{t(cat.label)}</span>
                    <div className="flex-1 h-px bg-gray-100" />
                    <span className={`text-[11px] font-bold tabular-nums ${catEnabled === cat.perms.length ? cat.color : "text-gray-300"}`}>
                        ...
                        <button
                          ...
                          className={`flex items-center justify-between px-4 py-3 rounded-xl border text-sm transition-all
                            ${isActive
                              ...
                              : "border-gray-100 bg-gray-50/70 hover:border-gray-200 hover:bg-gray-100/60"
                            }
                            ${locked ? "cursor-default" : "cursor-pointer active:scale-[0.99] hover:shadow-sm"}`}
                        >
                          <span className={`font-semibold text-[13px] text-left leading-tight ${isActive ? "text-violet-700" : "text-gray-400"}`}>
                            {t(perm)}
                          </span>
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ml-2 transition-all
                            ${isActive ? "bg-violet-500 shadow-sm shadow-violet-300/50" : "bg-gray-200"}`}>
                            {isActive
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h2 className="font-bold text-gray-900">{t("Create New Role")}</h2>
            ...
            <form onSubmit={handleCreate} className="px-6 py-5 space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{t("Spaces will be converted to underscores.")}</p>
              </div>
              <div>
                    ...
                    return (
                      <button type="button" key={perm} onClick={() => toggleNewPerm(perm)}
                        className={`flex items-center justify-between px-3 py-2 rounded-lg border text-xs transition-all ${
                          active ? "border-violet-200 bg-violet-50 text-violet-700" : "border-gray-100 bg-gray-50 text-gray-500 hover:bg-gray-100"
                        }`}>
                        <span className="font-medium text-left">{perm}</span>
                        {active ? <Check size={13} className="text-violet-500 flex-shrink-0" /> : <X size={13} className="text-gray-300 flex-shrink-0" />}
                      </button>
                    );
                  })}
                </div>
                <p className="text-xs text-gray-400 mt-2">{newRolePerms.length} {t("of")} {PERMISSIONS.length} {t("selected")}</p>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-1">{t("Delete")} "{deleteTarget}" {t("role?")}</h3>
